...
If another session only needs the short "make this SMIKE-ready" contract, start with `scripts/smike/SPEC_PREP_BRIEF.md`. Use this document when you need the full parser-facing reference or the longer prompt for a non-SMIKE planning session.

A spec handed to `./smike <spec.md> [context.md ...]` is parsed by `cli.mjs` → `buildPlanningBundle` (heading-exact). Only the first file is parsed as the spec body. Later files are context refs only: they can enrich `primary_refs`, but they do not satisfy required headings like `## Objective`, `## Required Deliverable From This Loop`, `## Required Planning Output Shape`, or `## Priority N:`.

`./smike` now runs a framework-owned spec-prep pass before normal planning promotion. When the main spec is close but not yet parser-safe, SMIKE can rewrite a managed contract block in the first file to normalize headings, phase lines, and proof commands. The first-file rule still applies: the final parser-facing contract still lives in the main spec file, not in attached context docs.

`./smike <spec...>` now bootstraps new projects in a draft planning state first. Draft bootstrap writes the planning bundle skeleton but skips checker/auditor gating until `./smike cycle <project>` can promote the bundle with concrete summaries and proof commands. Once promoted, the normal planning bundle includes `PLAN.json`, `PLAN-GRAPH.json`, phase plans, checker + auditor, and the runtime dispatch view embedded in `STATE.json`.

If you only have a short request, use `./smike "<prompt>" [--context=path1,path2] [--spec=memories/name.md]` or `./smike intake "<prompt>" --context=path1,path2 --spec=memories/name.md`. SMIKE auto-routes that into a draft spec, including `## Intake Prompt` and `## Clarifying Questions`, then enters the same `planning_draft` flow described below.

## Planning design prompt

If you want a model to turn a vague request into a spec that SMIKE can ingest cleanly, give it this prompt:

```markdown
Expand this request into a single self-contained SMIKE spec that preserves exact parser headings.

Use attached docs only as source material and truth refs. Put every required planning section in the main spec file itself; do not rely on context docs to supply `## Objective`, `## Required Deliverable From This Loop`, `## Required Planning Output Shape`, or `## Priority N:` sections.

SMIKE may refresh a managed `SMIKE:CONTRACT` block before planning, but the main spec still has to carry real repo-aware intent. During `planning_draft`, update the spec file and rerun the cycle; do not patch generated `.smike/**` artifacts.

Required shape:
- `## Objective`: one concrete outcome.
- `## What The Planner Must Read First`: canonical repo paths the planner should treat as truth.
- `## Critical Constraints`, `## Explicit Non-Goals`, `## Explicit Deferrals`, and `## Protected / High-Collision Areas` when those boundaries matter.
- `## Required Deliverable From This Loop` or `## Planner Must Produce`: only checker-auditable outputs.
- `## Required Planning Output Shape`: concrete `- Plan NN:` lines. Each phase should include `category:` or `write_scope:`, explicit `depends:` when the default chain is wrong, and phase-specific `verify:` commands for code-bearing work once the surface is concrete.
- Matching `## Priority N:` summaries for every phase, written with enough detail to become phase scope.

Avoid fallback scaffolding. Replace generic phase titles, generic proof commands, and broad "implement the feature" summaries with narrow repo-specific slices. Leave `## Clarifying Questions` only for unresolved facts that block promotion; otherwise record assumptions and proceed.
```

This is already the direction of the autogenerated intake draft in `cli.mjs`; the point of the prompt is to make that intent explicit for humans or child models refining the draft.

For non-SMIKE planning sessions, the output should be the spec file content plus any suggested `./smike <spec.md> [context.md ...]` invocation. The planner should not create or edit `.smike/**`; those artifacts are owned by the SMIKE runtime.
...
`./smike <spec.md> [context.md ...]` is not a multi-file spec merge.

- `spec.md` must be self-contained for all load-bearing planning headings.
- `context.md ...` files are read as supporting refs and truth sources only.
- If `## Objective` exists only in a context file, lint still reports the spec as missing `Objective`.
- If deliverables, phase blueprints, or `## Priority N:` sections live only in context files, planning still fails.

Use context files for background, source material, or large supporting plans. Use the main spec for the actual planning contract.
...
| `## Critical Constraints` | `constraints` | strategist uses these when refining the planning contract |
